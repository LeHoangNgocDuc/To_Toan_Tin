...
const SchedulePage: React.FC<SchedulePageProps> = ({ user }) => {
  ...
  const handleCellClick = (day: number, period: number, currentItem?: ScheduleItem) => {
    // Determine subject to show. 
    // If editing, use item subject.
    // If new, try to use user's subject, but be careful not to create a bad initial string.
    let initSubject = user.subject;
    let initClass = '';

    if (currentItem) {
        initSubject = currentItem.subject;
        initClass = currentItem.className;
    }
  ...
  const handleAssignmentChange = (val: string) => {
    if (!val) {
        setNewEntry(prev => ({...prev, className: ''}));
        return;
    }
    // FIX: Sử dụng lastIndexOf để tách chuỗi chính xác cho môn học có dấu cách (VD: Tin học, Công nghệ)
    // Format luôn là "[Môn học] [Lớp]" -> Lớp luôn nằm sau khoảng trắng cuối cùng.
    const lastSpaceIndex = val.lastIndexOf(' ');
    
    if (lastSpaceIndex !== -1) {
        const sub = val.substring(0, lastSpaceIndex);
        const cls = val.substring(lastSpaceIndex + 1);
        setNewEntry(prev => ({...prev, subject: sub, className: cls}));
    } else {
        // Trường hợp lỗi format hoặc chỉ có tên môn
        setNewEntry(prev => ({...prev, subject: val, className: ''}));
    }
  };
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select 
                    value={newEntry.className ? `${newEntry.subject} ${newEntry.className}` : ""} 
